...
safety-critical integrity statement.
\end{abstract}
...
The pipeline contains a thoughtful multi-stage float-to-fixed design and more
screening than a minimal RTK implementation.  Nevertheless, the current FIXED
decision should not yet be treated as integrity-safe.  Shared-reference
correlation, non-transactional wide-lane commitment, incomplete fixed-factor
lifecycle management, and cross-satellite Hatch statistics are the priority
issues.  Addressing those four items and validating with injected faults would
substantially improve both correctness and operational robustness.
